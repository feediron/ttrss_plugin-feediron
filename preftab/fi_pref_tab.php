...
class Feediron_PrefTab{
	public static function get_pref_tab($json_conf, $test_conf){
		...
		$tab .= '<a href="https://github.com/feediron/ttrss_plugin-feediron/blob/master/README.md">Configuration help</a>';
		$tab .= self::get_form_start('save');
		$tab .= self::get_script('responseJSON.message','dojo.query("#json_conf").attr("value",responseJSON.json_conf); dojo.query("#json_error").attr("innerHTML", "").attr("class",""); ','dojo.query("#json_error").attr("innerHTML", responseJSON.json_error).attr("class","error");');
		...
		$tab .= self::get_script('responseJSON.message','dojo.query("#json_conf").attr("value",responseJSON.json_conf); ');
		...
		$tab .= self::get_script('responseJSON.message','console.log(transport); dojo.query("#json_export").attr("innerHTML",responseJSON.json_export);
		...
		$tab .= self::get_script('"Updated"','dojo.query("#test_url").attr("innerHTML", "<pre>"+responseJSON.url+"</pre>"); dojo.query("#test_result").attr("innerHTML", responseJSON.content); dojo.query("#test_log").attr("innerHTML", responseJSON.log.join("\n")); dojo.query("#test_conf").attr("value", responseJSON.config);');
	...
	private static function get_script($notification, $successaction,$failaction=''){
      ...
      xhrPost("backend.php", this.getValues(), (transport) => {
        ...
        if (responseJSON.success == false){
          Notify.error(responseJSON.errormessage);
          '.$failaction.'
        } else {
