...
class fi_mod_readability
{
  ...
  public function perform_filter( $html, $config, $settings ){
    ...
    if (class_exists(ReadabilityPHP::class)) {
      ...
      if( array_key_exists($config, 'prependexcerpt' ) && ( $config['prependexcerpt'] ) && !( $config['excerpt'] )  ) {
      ...
      if( array_key_exists($config, 'prependimage' ) && ( $config['prependimage'] )  ) {
      ...
      if( array_key_exists($config, 'appendimages') && ( $config['apendimages'] )  ) {
        $images = $readability->getImages();
        foreach ( $images as $image ) {
          $content.='<img src="'.$image.'"></img><br>';
        }
    ...
    if (array_key_exists($config, 'xpath')){
